...
async function fetchWithTimeout(
	...
	try {
		...
		t = setTimeout( () => controller.abort(), ms );
...
async function fetchWalletDetails(
	...
	if ( ! res.ok ) {
		throw new Error( 'Wallet verification failed for ' + url );
	}
		...
		try {
			...
			const results = await Promise.all(
				pointers.map( async ( p ) => {
					...
					const { url, response } =
						await fetchWalletDetails( pointerUrl );
...
document.addEventListener( 'DOMContentLoaded', () => {
	...
	if ( form ) {
		...
		const compare = () => {
			...
			if (
				current.entries().next().done &&
				initial.entries().next().done
			) {
				return false;
			}
			// Simple diff
	...
	if ( table && addBtn ) {
		addBtn.addEventListener( 'click', () => {
			...
			const par =
				newRow.querySelector< HTMLElement >( '.wallet-ui-wrapper' );
			if ( par ) {
				par.innerHTML = '';
			}
			newRow
