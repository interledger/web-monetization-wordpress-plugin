...
class Frontend {
	...
	public function add_monetization_atom_link_to_feed_item(): void {
		...
		$link_tag = $this->generate_monetization_link_for_post( $post->ID, 'atom:link', array( 'site' ) );
		if ( $link_tag ) {
	 ...
	 */
	public function generate_monetization_link_for_post( $post_id, $element_type = 'link', array $excludes = array() ): ?string {
		...
		$wallets = $this->get_wallets_for_post( $post, $excludes );
		if ( empty( $wallets['list'] ) ) {
			return null;
		}
	...
	private function get_wallets_for_post( $post, $excludes = array() ): array {
		...
		// Site wallet.
		if ( ! in_array( 'site', $excludes, true ) ) {
