...
class Frontend {
	public function register_hooks(): void {
		add_action( 'wp_enqueue_scripts', [ $this, 'enqueue_wm_banner_script' ] );
		add_action( 'wp_head', [ $this, 'render_web_monetization_link' ] );

		// Feeds
		add_action( 'rss2_head', [ $this, 'add_monetization_atom_link_to_feed_head' ] );
		add_action( 'rss2_item', [ $this, 'add_monetization_atom_link_to_feed_item' ] );
		add_action( 'atom_entry', [ $this, 'add_monetization_link_to_feed_item' ] );
		add_action( 'atom_head', [ $this, 'add_site_monetization_link_to_feed_head' ] );
	}
	...
	public function add_monetization_atom_link_to_feed_item(): void {
		global $post;
		if ( ! $post instanceof \WP_Post || ! $this->is_enabled() ) return;
	...
	public function generate_monetization_link_for_post( $post_id, $element_type = 'link' ): ?string {
		...
		if ( empty( $wallets['list'] ) ) return null;
		...
		if ( $mode === 'all' ) {
			...
			foreach ( [ 'article', 'author', 'post_type', 'site' ] as $key ) {
	...
	private function get_wallets_for_post( $post ): array {
		...
		if ( ! $author_disabled ) {
			...
			if ( $post_wallet ) $list['article'] = $post_wallet;

			// Author wallet
			if ( get_option( 'wm_enable_authors', false ) ) {
				$author_wallet = get_user_meta( $post->post_author, 'wm_wallet_address', true );
				if ( $author_wallet ) $list['author'] = $author_wallet;
			}
		}

		// Post type wallet
		$post_type_wallets = get_option( 'wm_post_type_settings', [] );
		...
		// Site-wide wallet
		$site_wallet = get_option( 'wm_wallet_address', '' );
		if ( $site_wallet ) $list['site'] = $site_wallet;
	...
	public function enqueue_wm_banner_script(): void {
		if ( is_attachment() || ( ! is_singular() && ! is_front_page() && ! is_author() ) ) return;
		if ( ! get_option( 'wm_banner_enabled', 1 ) || ! $this->is_enabled() ) return;
		...
		wp_localize_script( 'wm-banner-script', 'wm', [
			'wmBannerConfig' => get_option( 'wm_banner_config', [] ),
